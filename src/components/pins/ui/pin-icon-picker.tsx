...
// Preset colors inspired by fantasy themes
const PRESET_COLORS = [
...
export function PinIconPicker({
  ...
  const uploadIconMutation = useMutation({
    mutationFn: async (file: File) => {
      ...
      const { uploadCustomPinIcon } = await import("@/actions/pins");
  ...
  const uploadBackgroundMutation = useMutation({
    mutationFn: async (file: File) => {
      ...
      formData.append("file", file);
      const { uploadCustomPinIcon } = await import("@/actions/pins");
      // Use same upload action but store as background
      return uploadCustomPinIcon(pinId, formData);
    },
    onSuccess: (result) => {
      if (result.success) {
...
function ShapePreview({ shape, color }: ShapePreviewProps) {
  const size = 32;
