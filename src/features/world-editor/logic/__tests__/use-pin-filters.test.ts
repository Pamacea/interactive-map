...
let mockVisibleTypes: string[];

describe("usePinFilters", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    // Default mock values - all types visible using actual PinType enum values
    mockFilters = {
      [PinType.CITY]: true,
  ...
  describe("isTypeVisible", () => {
    ...
    it("should default to true for undefined filter", () => {
      delete mockFilters[PinType.CUSTOM];

      const { result } = renderHook(() => usePinFilters());

      expect(result.current.isTypeVisible(PinType.CUSTOM)).toBe(true);
    });
  });
...
describe("useIsPinTypeVisible", () => {
  ...
  it("should default to true for undefined filter", () => {
    ...
    const { result } = renderHook(() => useIsPinTypeVisible(PinType.CUSTOM));
...
describe("useFilteredPins", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mockVisibleTypes = [PinType.CITY, PinType.VILLAGE];
  });
...
describe("useVisiblePinTypesCount", () => {
  ...
  it("should return count of visible types", () => {
    mockVisibleTypes = [PinType.CITY, PinType.VILLAGE, PinType.DUNGEON];
