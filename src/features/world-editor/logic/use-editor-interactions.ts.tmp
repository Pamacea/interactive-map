...
export function useEditorInteractions(config: EditorInteractionsConfig = {}) {
  ...
  // Store shortcuts methods to avoid dependency issues
  const shortcutsMethods = shortcuts.shortcuts;
  const actions = {
    /** Manually trigger pin deletion with toast */
    deletePin: useCallback(async () => {
      if (shortcuts.selectedPinId) {
        await shortcutsMethods.delete();
      }
    }, [shortcuts.selectedPinId, shortcutsMethods]),

    /** Manually clear selection with toast */
    clearSelection: useCallback(() => {
      shortcutsMethods.escape();
    }, [shortcutsMethods]),

    /** Manually trigger undo with toast */
    undo: useCallback(() => {
      shortcutsMethods.undo();
    }, [shortcutsMethods]),

    /** Manually trigger redo with toast */
    redo: useCallback(() => {
      shortcutsMethods.redo();
    }, [shortcutsMethods]),
  };
