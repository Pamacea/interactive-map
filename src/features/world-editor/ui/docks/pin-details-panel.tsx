...
import { MapPin, Tag, Link2, GripVertical, Settings, Layers, Check, Image as ImageIcon } from "lucide-react";
...
import { PinGallerySection } from "@/features/pins/ui/gallery-section";
import { PinIconPicker } from "@/features/pins/ui/icon-picker/pin-icon-picker";
...
export function PinDetailsPanel({ pin, worldId }: PinDetailsPanelProps) {
  ...
  const updatePinServer = React.useCallback(async (data: Parameters<typeof import("@/features/pins/actions").updatePin>[0]) => {
    const { updatePin: updatePinAction } = await import("@/features/pins/actions");
...
function ReferencesSection({ pinId, description, worldId: _worldId }: ReferencesSectionProps) {
  ...
  const { data: relations = [] } = useQuery({
    ...
    queryFn: async () => {
      const { getPinTagRelations } = await import("@/features/world-editor/actions");
...
function PinGalleryCollapsibleSection({ pin, worldId }: PinGalleryCollapsibleSectionProps) {
  ...
  const { data: galleryImages = [] } = useQuery({
    ...
    queryFn: async () => {
      const { getGalleryItemsByPin } = await import("@/features/gallery/actions");
