...
import { MapPin, Image as ImageIcon, User, FileText } from "lucide-react";
...
export function TagAutosuggestTextarea({
  ...
  const { data: pins = [] } = useQuery({
    ...
    queryFn: async () => {
      const { getPinsByWorld } = await import("@/features/pins");
      return getPinsByWorld(worldId);
    },
    enabled: !!worldId && showSuggestions,
    staleTime: 1000 * 60 * 5, // 5 minutes - suggestions don't change often
  ...
  const { data: galleryItems = [] } = useQuery({
    ...
    queryFn: async () => {
      const { getGalleryItemsByWorld } = await import("@/features/gallery");
  ...
  const { data: loreEntries = [] } = useQuery({
    ...
    queryFn: async () => {
      const { getLoreEntriesByWorld } = await import("@/features/lore");
  ...
  const { data: characters = [] } = useQuery({
    ...
    queryFn: async () => {
      const { getCharactersByWorld } = await import("@/features/characters");
  ...
  const suggestions = React.useMemo((): SuggestionItem[] => {
    ...
    // Add pins (with slug)
    pins.forEach((pin: { id: string; title: string; slug?: string }) => {
    ...
    characters.forEach((char: { id: string; name: string; slug?: string }) => {
    ...
    // Add lore (with slug)
    loreEntries.forEach((entry: { id: string; title: string; slug?: string }) => {
    ...
    galleryItems.forEach((item: { id: string; title?: string }) => {
