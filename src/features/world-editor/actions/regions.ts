...
import type { Region, RegionCoordinates } from "@/types/region.type";
 ...
 * @returns Result with updated region or error
 */
export async function toggleRegionVisibility(regionId: string): Promise<Result<Region>> {
  return safeAsync(async () => {
    const user = await getAuthenticatedUser();
...
): Promise<Result<Pin | GalleryItem | Region>> {
  return safeAsync(async () => {
    const user = await getAuthenticatedUser();
