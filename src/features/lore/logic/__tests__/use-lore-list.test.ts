...
describe("useLoreList", () => {
  ...
  describe("with initial data", () => {
    ...
    it("should not fetch when initial data is provided", () => {
      const initialData = [{ id: "1", title: "Lore 1" }] as any[];
  ...
  describe("fetching", () => {
    it("should fetch lore entries when enabled", async () => {
      ...
      (getLoreEntriesByWorld as any).mockResolvedValue(loreData);
    ...
    it("should set loading state during fetch", async () => {
      vi.useRealTimers(); // Disable fake timers for async test
      (getLoreEntriesByWorld as any).mockImplementation(
        () => new Promise((resolve) => {
    ...
    it("should handle fetch errors", async () => {
      ...
      (getLoreEntriesByWorld as any).mockRejectedValue(error);
    ...
    it("should handle non-error objects", async () => {
      vi.useRealTimers(); // Disable fake timers for async test
      (getLoreEntriesByWorld as any).mockRejectedValue("String error");
  ...
  describe("refetch", () => {
    it("should refetch lore entries", async () => {
      vi.useRealTimers(); // Disable fake timers for async test
      const loreData = [{ id: "1", title: "New Lore" }] as any[];
