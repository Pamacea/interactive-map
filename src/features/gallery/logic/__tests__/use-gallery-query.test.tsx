...
const createWrapper = () => {
  ...
  return function TestQueryProvider({ children }: { children: React.ReactNode }) {
...
describe("useGallery", () => {
  ...
  it("should fetch gallery items for a world", async () => {
    ...
    (getGalleryItemsByWorld as ReturnType<typeof vi.fn>).mockResolvedValue(mockData);
...
describe("usePinGallery", () => {
  ...
  it("should fetch gallery items for a pin", async () => {
    const mockData = [{ id: "1", url: "image1.jpg" }] as unknown[];
    (getGalleryItemsByWorld as ReturnType<typeof vi.fn>).mockResolvedValue(mockData);
...
describe("useUploadGallery", () => {
  ...
  it("should upload gallery image", async () => {
    ...
    (uploadGalleryImage as unknown).mockResolvedValue(mockResult);
...
describe("useDeleteGallery", () => {
  ...
  it("should delete gallery item", async () => {
    (deleteGalleryItem as unknown).mockResolvedValue({ success: true });
...
describe("useUpdateGallery", () => {
  ...
  it("should update gallery item", async () => {
    (updateGalleryItem as unknown).mockResolvedValue({ success: true });
